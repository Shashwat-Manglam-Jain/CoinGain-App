...
export default function AdminDashboard({ navigation }) {
  ...
  // Load admin user and data from backend
  useEffect(() => {
  ...
  const fetchNotifications = useCallback(async (adminId) => {
    try {
      ...
      const token = await AsyncStorage.getItem('userToken');
      const response = await axios.get(`${API_BASE_URL}/datafetch/notifications/${adminId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchRedemptions = useCallback(async (adminId) => {
    try {
      ...
      const token = await AsyncStorage.getItem('userToken');
      const response = await axios.get(`${API_BASE_URL}/datafetch/redemptions/${adminId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSaveUser = async () => {
    try {
      const token = await AsyncStorage.getItem('userToken');
      await axios.put(`${API_BASE_URL}/fetchdata/user/${editUser._id}`, editUser, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSendTokens = async (userId) => {
    ...
    setModalAction(() => async () => {
      try {
        ...
        setUsers((prev) =>
          prev.map((u) =>
            u._id === userId ? { ...u, points: u.points + parseInt(tokenAmount) } : u
          )
        );
        ...
        Alert.alert('Error', error.response?.data?.error || 'Failed to send tokens');
      }
    });
    setModalVisible(true);
  };
  ...
  const handleSendCoins = async () => {
    ...
    const user = users.find((u) => u.name === userId || u._id === userId || u.mobile === userId || u.uniqueCode === userId);
    if (!user) {
      Alert.alert('Error', 'User not found');
      return;
    }
    setModalMessage(`Are you sure you want to send ${amount} coins to ${user.name}?`);
    setModalAction(() => async () => {
      try {
        const token = await AsyncStorage.getItem('userToken');
        const response = await axios.post(
        ...
        setUsers((prev) =>
          prev.map((u) =>
            u._id === user._id ? { ...u, points: u.points + parseInt(amount) } : u
          )
        );
        setNotifications((prev) => [
          ...prev,
  ...
  const handleAddReward = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('userToken');
      const response = await axios.post(
        `${API_BASE_URL}/fetchdata/admin/${adminUser._id}/reward`,
        {
          name: newReward.name,
      ...
      Alert.alert('Error', error.response?.data?.error || 'Failed to add reward');
    } finally {
  ...
  const handleDeleteReward = async (rewardId) => {
    ...
    setModalAction(() => async () => {
      try {
        ...
        Alert.alert('Error', error.response?.data?.error || 'Failed to delete reward');
      } finally {
  ...
  const handleClearNotification = async (notificationId) => {
    try {
      const token = await AsyncStorage.getItem('userToken');
      await axios.delete(`${API_BASE_URL}/fetchdata/notification/${notificationId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleApproveRedemption = async (redemptionId) => {
    try {
      ...
      setRedemptions((prev) => prev.map((r) => (r._id === redemptionId ? { ...r, status: 'approved' } : r)));
    } catch (error) {
  ...
  const handleRejectRedemption = async (redemptionId) => {
    try {
      ...
      setRedemptions((prev) => prev.map((r) => (r._id === redemptionId ? { ...r, status: 'rejected' } : r)));
    } catch (error) {
          ...
          <>
            ...
            <View style={[styles.card, { backgroundColor: isDarkMode ? '#1e1e1e' : '#fff' }]}>
              <Card.Content>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setCoinSettings({ ...coinSettings, userId: text })}
                  style={styles.input}
                ...
                <TextInput
                  ...
                  onChangeText={(text) => setCoinSettings({ ...coinSettings, amount: text })}
                  style={styles.input}
                  theme={{ colors: { text: colors.text, primary: colors.primary } }}
                  mode="outlined"
            ...
            <View style={[styles.card, { backgroundColor: isDarkMode ? '#1e1e1e' : '#fff' }]}>
              <Card.Title
                title="Rewards"
                titleStyle={[styles.cardTitle, { color: colors.text }]}
                left={() => <Avatar.Icon size={40} icon="gift" style={{ backgroundColor: colors.primary }} />}
              />
              <Card.Content>
                        ...
                        <View style={styles.buttonRow}>
                          ...
                          <View style={styles.buttonContainer}>
                            <Button
                              ...
                              style={styles.actionButton}
                              textColor={colors.text}
                              onPress={() => handleButtonPress(() => setEditUser(null))}
                            >
                              <ButtonText>Cancel</ButtonText>
                            </Button>
                          </View>
                        ...
                        <View style={styles.buttonRow}>
                          <View style={styles.buttonContainer}>
                            <Button
                              ...
                              buttonColor={colors.primary}
                              textColor="#fff"
                              onPress={() => handleButtonPress(() => handleSendTokens(item._id))}
                            >
                          ...
                          <View style={styles.buttonContainer}>
                            <Button
                              ...
                              style={styles.actionButton}
                              textColor={colors.text}
                              onPress={() => handleButtonPress(() => setSendTokenUserId(null))}
                            >
                              <ButtonText>Cancel</ButtonText>
                            </Button>
                          </View>
                        ...
                        <View style={styles.buttonRow}>
                          <View style={styles.buttonContainer}>
                            <Button
                              ...
                              onPress={() => handleButtonPress(() => handleEditUser(item))}
                            >
                          ...
                          <View style={styles.buttonContainer}>
                            <Button
                              ...
                              onPress={() => handleButtonPress(() => handleDeleteUser(item._id))}
                            >
                          ...
                          <View style={styles.buttonContainer}>
                            <Button
                              ...
                              buttonColor={colors.primary}
                              textColor="#fff"
                              onPress={() => handleButtonPress(() => setSendTokenUserId(item._id))}
                            >
          ...
          <>
            ...
            <View style={[styles.card, { backgroundColor: isDarkMode ? '#1e1e1e' : '#fff' }]}>
              <Card.Title
                ...
                titleStyle={[styles.cardTitle, { color: colors.text }]}
                left={() => <Avatar.Icon size={40} icon="ticket" style={{ backgroundColor: colors.primary }} />}
              />
              <Card.Content>
                <FlatList
                  ...
                  ListEmptyComponent={() => (
                    <Text style={[styles.emptyText, { color: colors.text }]}>No pending redemptions.</Text>
                  )}
                  initialNumToRender={10}
                  maxToRenderPerBatch={10}
                  windowSize={5}
            ...
            <View style={[styles.card, { backgroundColor: isDarkMode ? '#1e1e1e' : '#fff' }]}>
              ...
              <Card.Content>
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <View
                      style={[
                        ...
                        { backgroundColor: item.read ? (isDarkMode ? '#2a2a2a' : '#f5f5f5') : (isDarkMode ? '#333' : '#fff') },
                      ]}
                    >
                      <Avatar.Icon
                        size={36}
                        icon={item.read ? 'bell-outline' : 'bell'}
                        style={{ backgroundColor: item.read ? colors.secondary : colors.primary, marginRight: 10 }}
                      />
                      <View style={{ flex: 1 }}>
                  ...
                  ListEmptyComponent={() => (
                    <Text style={[styles.emptyText, { color: colors.text }]}>No notifications available.</Text>
                  )}
                  initialNumToRender={10}
                  maxToRenderPerBatch={10}
                  windowSize={5}
          ...
          <>
            ...
            <View style={[styles.card, { backgroundColor: isDarkMode ? '#1e1e1e' : '#fff' }]}>
              <Card.Title
                ...
                titleStyle={[styles.cardTitle, { color: colors.text }]}
                left={() => <Avatar.Icon size={40} icon="chart-bar" style={{ backgroundColor: colors.primary }} />}
              />
              <Card.Content>
                <View style={styles.statContainer}>
                  <View style={styles.statItem}>
                    <MaterialIcons name="people" size={28} color={colors.primary} style={styles.statIcon} />
                    <Text style={[styles.cardText, { color: colors.text }]}>
                      Total Users: {users.length}
                    </Text>
                  </View>
                  <View style={styles.statItem}>
                    <MaterialCommunityIcons name="coin" size={28} color={colors.primary} style={styles.statIcon} />
                    <Text style={[styles.cardText, { color: colors.text }]}>
                      Total Coins Sent: {users.reduce((sum, u) => sum + (u.points || 0), 0)}
                    </Text>
                  </View>
                  <View style={styles.statItem}>
                    <MaterialIcons name="notifications" size={28} color={colors.primary} style={styles.statIcon} />
                    <Text style={[styles.cardText, { color: colors.text }]}>
                      Pending Redemptions: {redemptions.filter((r) => r.status === 'pending').length}
                    </Text>
                  </View>
                  <View style={styles.statItem}>
                    <MaterialCommunityIcons name="wallet-giftcard" size={28} color={colors.primary} style={styles.statIcon} />
                    <Text style={[styles.cardText, { color: colors.text }]}>
                      Available Rewards: {rewards.length}
                    </Text>
                  </View>
                </View>
                <Text style={[styles.sectionTitle, { color: colors.text }]}>User Progress</Text>
                <FlatList
                  data={users}
                  keyExtractor={(user) => user._id}
                  renderItem={({ item: user }) => {
                    const maxPoints = Math.max(...rewards.map((r) => parseInt(r.pointsRequired) || 100), 100);
                    const progress = Math.min((user.points / maxPoints) * 100, 100);
                    return (
                      <View style={styles.progressContainer}>
                        ...
                        <Text style={[styles.progressText, { color: colors.text }]}>
                          {user.points}/{maxPoints} points ({progress.toFixed(1)}%)
                        </Text>
                      </View>
                    );
                  }}
